...
export default function SecurityPage() {
  ...
  const submitFraudQuiz = () => setQuizSubmitted(true);

  const fraudScore = quizSubmitted ? quizQuestions.reduce((acc, q, i) => {
    return acc + (quizAnswers[i] === q.correct_answer ? 1 : 0);
  }, 0) : 0;

  const startSpendingQuiz = async () => {
    setSpendingLoading(true);
    setSpendingSubmitted(false);
    setSpendingAnswers({});
    try {
  ...
  const submitSpendingQuiz = () => setSpendingSubmitted(true);

  const spendingScore = spendingSubmitted ? spendingQuiz.reduce((acc, q, i) => {
    return acc + (spendingAnswers[i] === q.correct_answer ? 1 : 0);
  }, 0) : 0;
  ...
  const runSimulation = async () => {
    ...
    try {
      const res = await fetch("/api/v1/education/simulate/fraud", {
        ...
        body: JSON.stringify({
          amount: simAmount, hour: simHour,
          is_foreign_ip: simForeign, category: simCategory,
          is_new_merchant: simNewMerchant,
        }),
      });
      ...
      <main className="container">
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          {[
            ...
            { key: "simulate" as const, label: "사기 시뮬레이터" },
          ].map(({ key, label }) => (
          ...
          <div className="grid grid-2">
            ...
            <div className="card">
              ...
              <p className="text-secondary" style={{ marginBottom: 16, fontSize: 14 }}>
                실제 거래 패턴 기반 — 이 거래는 정상일까, 사기일까?
              </p>
              {quizQuestions.length === 0 && (
                <button onClick={startFraudQuiz} disabled={quizLoading} style={{
              ...
              {quizQuestions.map((q: any, i: number) => (
                <div key={i} style={{
                  padding: "14px 16px", margin: "10px 0", borderRadius: 10,
                  border: "1px solid var(--border)",
                  background: quizSubmitted
                    ? quizAnswers[i] === q.correct_answer ? "#f0fdf4" : "#fef2f2"
                    : "#fff",
                }}>
                  <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 14 }}>
                    Q{i + 1}. {q.transaction_summary}
                  </div>
                  <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                    {q.options.map((opt: string) => (
                      <button key={opt} onClick={() => !quizSubmitted && setQuizAnswers({ ...quizAnswers, [i]: opt })}
                        ...
                        style={{
                          ...
                          background: quizSubmitted && opt === q.correct_answer ? "#dcfce7"
                            : quizSubmitted && quizAnswers[i] === opt && opt !== q.correct_answer ? "#fecaca"
                            : quizAnswers[i] === opt ? "#eef2ff" : "#fff",
                  ...
                  {quizSubmitted && (
                    <div style={{
                      ...
                      {q.triggered_rules.length > 0 && (
                        <div style={{ marginTop: 4 }}>
                          <strong>감지 규칙:</strong> {q.triggered_rules.join(", ")}
                        </div>
                      )}
                    </div>
                  )}
              ...
              {quizQuestions.length > 0 && !quizSubmitted && (
                <button onClick={submitFraudQuiz} disabled={Object.keys(quizAnswers).length < quizQuestions.length}
                  style={{
                    width: "100%", marginTop: 12, padding: "10px", borderRadius: 8,
                    border: "none", background: Object.keys(quizAnswers).length < quizQuestions.length ? "#94a3b8" : "var(--primary)",
              ...
              {quizSubmitted && (
                <div style={{ textAlign: "center", marginTop: 16, padding: 16, borderRadius: 10, background: "#f8fafc" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: fraudScore >= 4 ? "var(--success)" : "var(--warning)" }}>
                    {fraudScore}/{quizQuestions.length} 정답
                  </div>
                  <p className="text-secondary" style={{ marginTop: 4 }}>
                    {fraudScore === quizQuestions.length ? "완벽합니다! 보안 전문가 수준이에요!" :
                     fraudScore >= 3 ? "좋습니다! 조금만 더 공부하면 완벽해요." :
                     "사기 패턴에 대해 더 학습이 필요합니다."}
                  </p>
                  <button onClick={startFraudQuiz} style={{
            ...
            <div className="card">
              ...
              <p className="text-secondary" style={{ marginBottom: 16, fontSize: 14 }}>
                예산, 저축, 투자, 보안 관련 금융 상식 퀴즈
              </p>
              {spendingQuiz.length === 0 && (
                <button onClick={startSpendingQuiz} disabled={spendingLoading} style={{
                  padding: "10px 24px", borderRadius: 8, border: "none",
              ...
              {spendingQuiz.map((q: any, i: number) => (
                <div key={i} style={{
                  padding: "14px 16px", margin: "10px 0", borderRadius: 10,
                  border: "1px solid var(--border)",
                  background: spendingSubmitted
                    ? spendingAnswers[i] === q.correct_answer ? "#f0fdf4" : "#fef2f2"
                    : "#fff",
                }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {q.options.map((opt: string) => (
                      <button key={opt}
                        onClick={() => !spendingSubmitted && setSpendingAnswers({ ...spendingAnswers, [i]: opt })}
                        disabled={spendingSubmitted}
                        style={{
                          padding: "10px 14px", borderRadius: 8, textAlign: "left",
                          fontSize: 14, cursor: spendingSubmitted ? "default" : "pointer",
                          border: spendingAnswers[i] === opt ? "2px solid var(--primary)" : "1px solid var(--border)",
                          background: spendingSubmitted && opt === q.correct_answer ? "#dcfce7"
                            : spendingSubmitted && spendingAnswers[i] === opt && opt !== q.correct_answer ? "#fecaca"
                            : spendingAnswers[i] === opt ? "#eef2ff" : "#fff",
                  ...
                  {spendingSubmitted && (
                    <div style={{
                      padding: "8px 12px", borderRadius: 6, fontSize: 13,
                      background: "#f8fafc", lineHeight: 1.6, marginTop: 8,
                    }}>
                      <strong>해설:</strong> {q.explanation}
                    </div>
                  )}
              ...
              {spendingQuiz.length > 0 && !spendingSubmitted && (
                <button onClick={submitSpendingQuiz}
                  disabled={Object.keys(spendingAnswers).length < spendingQuiz.length}
                  style={{
                    width: "100%", marginTop: 12, padding: "10px", borderRadius: 8,
                    border: "none",
                    background: Object.keys(spendingAnswers).length < spendingQuiz.length ? "#94a3b8" : "#22c55e",
                    color: "#fff", cursor: "pointer",
                  }}>
                  제출하기
                </button>
              ...
              {spendingSubmitted && (
                <div style={{ textAlign: "center", marginTop: 16, padding: 16, borderRadius: 10, background: "#f8fafc" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: spendingScore >= 4 ? "var(--success)" : "var(--warning)" }}>
                    {spendingScore}/{spendingQuiz.length} 정답
                  </div>
                  <button onClick={startSpendingQuiz} style={{
                    marginTop: 12, padding: "8px 20px", borderRadius: 8,
          ...
          <div className="grid grid-2">
            <div className="card">
              ...
              <p className="text-secondary" style={{ marginBottom: 20, fontSize: 14 }}>
                가상 거래 조건을 설정하고, 사기 탐지 시스템이 어떻게 판정하는지 확인하세요.
              </p>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="range" min={0} max={10000000} step={100000} value={simAmount}
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--text-secondary)" }}>
                  <span>0시</span>
                  <span style={{ color: 2 <= simHour && simHour <= 5 ? "var(--danger)" : "inherit", fontWeight: 2 <= simHour && simHour <= 5 ? 600 : 400 }}>
                    {2 <= simHour && simHour <= 5 ? "심야 시간대" : ""}
                  </span>
                  <span>23시</span>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                <button onClick={() => { setSimAmount(4500); setSimHour(12); setSimForeign(false); setSimNewMerchant(false); }}
                  style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "#fff", cursor: "pointer", fontSize: 12 }}>
                  일반 점심
                </button>
                <button onClick={() => { setSimAmount(1500000); setSimHour(3); setSimForeign(true); setSimNewMerchant(true); }}
                  style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid var(--danger)", background: "#fef2f2", cursor: "pointer", fontSize: 12, color: "var(--danger)" }}>
                  고위험 시나리오
                </button>
                <button onClick={() => { setSimAmount(500000); setSimHour(9); setSimForeign(false); setSimNewMerchant(false); setSimCategory("finance"); }}
                  style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "#fff", cursor: "pointer", fontSize: 12 }}>
                  적금 이체
                </button>
              </div>
              ...
              <button onClick={runSimulation} disabled={simLoading} style={{
                ...
                background: "var(--primary)", color: "#fff", cursor: "pointer", fontSize: 15,
              }}>
            ...
            {simResult && actionConf ? (
              <div className="card">
                ...
                <div style={{
                  ...
                  background: actionConf.bg, marginBottom: 20,
                }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: actionConf.color }}>
                    {actionConf.label}
                  </div>
                  <p style={{ marginTop: 8, fontSize: 14, color: "var(--text-secondary)" }}>
                    {simResult.risk_explanation}
                  </p>
                </div>
                ...
                {simResult.rules_triggered.map((r: any, i: number) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      borderLeft: `3px solid ${rConf.color}`, background: rConf.bg,
                    }}>
