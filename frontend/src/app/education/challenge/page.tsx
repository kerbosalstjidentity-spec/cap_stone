...
const BADGE_ICONS: Record<string, string> = {
  coffee: "☕", shield: "🛡️", star: "⭐", lock: "🔒", refresh: "🔄",
};
      ...
      <main className="container">
          ...
          <div className="card">
            ...
            {userChallenges.length > 0 ? (
              userChallenges.map((uc: any) => {
                ...
                return (
                  <div key={uc.challenge_id} style={{ padding: "14px 16px", margin: "8px 0", borderRadius: 10, border: "1px solid var(--border)", background: "#fff" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <span style={{ fontWeight: 600 }}>{ch?.name || uc.challenge_id}</span>
                      <span style={{ padding: "3px 10px", borderRadius: 12, fontSize: 12, fontWeight: 600, background: status.bg, color: status.color }}>{status.label}</span>
                    </div>
                    <div style={{ height: 8, background: "#e2e8f0", borderRadius: 4, overflow: "hidden", marginBottom: 6 }}>
                      <div style={{ height: "100%", borderRadius: 4, background: status.color, width: `${uc.progress_pct}%`, transition: "width 0.3s" }} />
                    </div>
                    <div className="text-secondary" style={{ fontSize: 12 }}>진행률 {uc.progress_pct.toFixed(0)}%</div>
                  </div>
                );
              })
            ) : (
              <p className="text-secondary" style={{ textAlign: "center", padding: 24 }}>참여 중인 챌린지가 없습니다.<br />아래에서 챌린지를 선택하세요!</p>
            )}
          </div>
        ...
        <div className="grid grid-3">
          {challenges.map((ch: any) => {
            ...
            return (
              <div key={ch.challenge_id} className="card" style={{ borderTop: enrolled ? "3px solid var(--success)" : "3px solid var(--primary)" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid var(--border)" }}>
                    {["순위", "사용자", "배지", "챌린지", "퀴즈 평균", "포인트"].map((h, i) => (
                      <th key={i} style={{ padding: "10px 12px", textAlign: i === 0 || i === 1 ? "left" : i === 5 ? "right" : "center", fontSize: 13 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {leaderboard.map((e: any) => (
                    <tr key={e.rank} style={{ borderBottom: "1px solid var(--border)", background: e.user_id === userId ? "#eef2ff" : "transparent" }}>
                      <td style={{ padding: "10px 12px", fontWeight: 600 }}>{e.rank <= 3 ? ["🥇", "🥈", "🥉"][e.rank - 1] : e.rank}</td>
                      <td style={{ padding: "10px 12px" }}>{e.user_id === userId ? <strong>{e.user_id} (나)</strong> : e.user_id}</td>
                      ...
                      <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 600, color: "var(--primary)" }}>{e.total_points}P</td>
                    </tr>
                  ))}
                </tbody>
              </table>
