...
export default function StrategyPage() {
  ...
  const fetchStrategy = async () => {
    ...
    else if (s.status === "rejected" || s.value?.detail) setError("전략 데이터를 불러올 수 없습니다.");
        ...
        <div className="grid grid-2">
              ...
              {strategy.saving_opportunities?.length > 0 ? (
                ...
                <div style={{ padding: 32, textAlign: "center" }}>
                  ...
                  <p className="text-secondary" style={{ marginTop: 12 }}>
                    현재 소비 패턴이 안정적입니다.<br />절약 가능 항목이 없습니다.
                  </p>
                </div>
              )}
            </div>
          ...
          {compare && (
            <div className="card" style={{ gridColumn: "1 / -1" }}>
              ...
              {/* 총액 비교 */}
              <div style={{
                margin: "20px 0", display: "flex", gap: 32, alignItems: "center",
                padding: "16px 24px", background: "#f8fafc", borderRadius: 10,
              }}>
                <div style={{ textAlign: "center" }}>
                  <div className="stat-label">{compare.previous_period}</div>
                  <div style={{ fontSize: 26, fontWeight: 700, color: "var(--text-secondary)" }}>
                    {formatKRW(compare.previous_total)}
                  </div>
                </div>
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 26, fontWeight: 700 }}>
                    {formatKRW(compare.current_total)}
                  </div>
                </div>
                <div style={{ flex: 1 }} />
                <div style={{ textAlign: "center" }}>
              ...
              <div style={{
                ...
                background: compare.total_diff_pct > 20
                  ? "#fef2f2"
                  : compare.total_diff_pct < -10 ? "#f0fdf4" : "#f8fafc",
                borderRadius: 8, fontSize: 15, lineHeight: 1.6,
                borderLeft: `4px solid ${compare.total_diff_pct > 20
                  ? "var(--danger)"
                  : compare.total_diff_pct < -10 ? "var(--success)" : "var(--border)"}`,
              }}>
                💡 {compare.insight}
              </div>
            </div>
          ...
          {!strategy && !loading && (
            <div className="card" style={{ gridColumn: "1 / -1", textAlign: "center", padding: 48 }}>
              ...
              <p className="text-secondary">사용자 ID를 입력하고 전략 조회를 눌러주세요.</p>
