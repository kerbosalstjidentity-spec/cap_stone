...
export default function DashboardPage() {
  ...
  const fetchProfile = async () => {
    ...
    try {
      const [profRes, trendRes] = await Promise.allSettled([
        fetch(`/api/v1/profile/${userId}`).then((r) => { if (!r.ok) throw new Error("not found"); return r.json(); }),
        fetch(`/api/v1/profile/${userId}/trend`).then((r) => r.ok ? r.json() : null),
      ]);
      if (profRes.status === "fulfilled") setProfile(profRes.value);
      else setError("프로필을 불러올 수 없습니다. 데모 데이터를 생성해보세요.");
      if (trendRes.status === "fulfilled" && trendRes.value) setTrend(trendRes.value.trend || []);
    } catch (e) {
  ...
  const seedDemo = async () => {
    setSeeding(true);
    try {
      const res = await fetch(`/api/v1/seed/demo/${userId}?months=3&tx_per_month=40`, { method: "POST" });
      if (res.ok) {
        await fetchProfile();
      }
    } finally {
  ...
  // 도넛 차트 데이터
  const pieData = profile?.category_breakdown.map((cat) => ({
