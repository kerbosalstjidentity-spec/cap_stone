...
def top_feature_reasons(
    ...
    *,
    ...
    W5-#5: 학습 시 산출한 ``mu``/``std`` 가 주어지면 단건 평가에서도 일관된
    z-score 가 산출돼 reason 설명이 안정화된다. 미지정 시 배치 통계 사용
    (기존 동작 호환).
    """
    if mu is None:
        mu = X.mean(axis=0)
    ...
    z = np.abs((X - np.asarray(mu, dtype=float)) / std)
...
def single_reason(
    ...
    *,
    ...
    """단건 1행 → reason_code 문자열. W5-#5: mu/std 옵션."""
    X = x_row.reshape(1, -1)
    return top_feature_reasons(X, feature_names, importances, top_k=top_k, mu=mu, std=std)[0]
...
def reason_code_to_human(
    ...
    *,
    ...
    W5-#5: 학습 시 mu/std 주입 시 단건 평가에서도 의미 있는 σ 값 산출.
    """
    X = x_row.reshape(1, -1)
    if mu is None:
        mu = X.mean(axis=0)
    ...
    std = np.maximum(np.asarray(std, dtype=float), 1e-9)
    imp = np.asarray(importances, dtype=float)
    z = np.abs((X[0] - np.asarray(mu, dtype=float)) / std)
    score = imp * (1.0 + z)
    idx = np.argsort(-score)[:top_k]
