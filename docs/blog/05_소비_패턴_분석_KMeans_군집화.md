...
- **인메모리 프로필 저장소 한계** — `InMemorySpendProfileStore`는 프로세스 재시작 시 휘발. 학습은 DB에서 다시 끌어오지만, 예측 API는 인메모리 집계에 의존하므로 재시작 직후 사용자가 0건으로 보일 수 있다. `spend_profile_db.py`로의 마이그레이션 흔적이 같은 디렉터리에 보이는 건 이 이슈를 알고 있다는 신호다. (✅ ROADMAP W2-#6 — `profile_store.rehydrate_from_db()` 추가, lifespan startup 에서 최근 180일 거래를 DB에서 자동 복원. DB가 단일 정본, 메모리는 분석 캐시. 추후 read path 전체를 `spend_profile_db` async API 로 이전 시 본 store 는 deprecation)
