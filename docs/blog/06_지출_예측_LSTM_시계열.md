...
- **`confidence` 가 휴리스틱** — LSTM 분기는 `0.7` 고정, WMA는 `0.3 + 0.1n`. 실제 예측 분산이나 잔차 기반 추정이 아니라 UI 라벨용이다. 신뢰도를 보고 액션을 거는 비즈니스 로직은 **이 값에 의존하지 말 것** — 잘못된 안전감을 줄 수 있다. (✅ ROADMAP W5-#4 — `predict(recent_months, mc_samples=N)` 입력 jitter 기반 의사 MC dropout, std/ci95_low/ci95_high 노출 + 신뢰도 = 0.9 - CV 클립 0.1~0.95, WMA fallback 도 가중 표준편차로 CI 산출)
