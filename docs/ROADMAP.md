...
- [x] **W5-#3** 사용자별 적응형 step-up 임계값 (전역 0.6 제거) — `03_Step-up:239` + `journey/04:102` + `threat/02:87` — [상/ML] — ✅ tbd (2026-05-10)
...
| 2026-05-10 | W5-#3 | 사용자별 적응형 step-up 임계값 (`app/services/stepup_threshold.py` clip(mean + k·std, 0.4, 0.9), 표본<min_history/std=0 → 글로벌 fallback, STEPUP_ADAPTIVE_* env, routes_stepup `/challenge` 가 record_risk_score + get_adaptive_threshold 사용) + 7 PASS | tbd |
