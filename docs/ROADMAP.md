...
- [x] **W5-#3** 사용자별 적응형 step-up 임계값 (전역 0.6 제거) — `03_Step-up:239` + `journey/04:102` + `threat/02:87` — [상/ML] — ✅ c0d7558 (2026-05-10)
- [x] **W5-#4** LSTM 신뢰도 → 예측 분산 추정 + 모델 가중치 영속화 — `06_LSTM:229,231` — [상/ML] — ✅ acd475b (2026-05-10)
...
- [x] **W6-#5** fds-research → fraud-service MLflow/DVC 버전 관리 + CI/CD — `20_학습:200` + `pipeline/02:94` + `mlops/02:90` — [상/인프라] — ✅ 4372836 (2026-05-10)
...
- [x] **W7-#9** ground truth precision/recall 비교 메트릭 — `18_AB:178` — [상/ML] — ✅ 1f88df5 (2026-05-10)
...
- [x] **W8-#2** 위협 인텔 외부 OSINT/상용 피드 연동 + 신뢰도 가중 — `journey/05:90` + `threat/05:88` — [상/거버넌스] — ✅ 33b7dea (2026-05-10)
...
- [x] **W8-#6** OR 결합 false-positive 비용 평가 + 정책 조정 — `threat/04:87` — [상/거버넌스] — ✅ c466f83 (2026-05-10)
- [x] **W8-#7** 비즈니스 KPI ↔ ML 지표 매핑 문서화 — `mlops/03:94` — [상/문서] — ✅ 5a6e979 (2026-05-10)
- [x] **W8-#8** revocation_manager.filter_attrs() 미들웨어 적용 — `15_ABAC_ABE:521` — [중/보안] — ✅ 3902bab (2026-05-10)
...
- [x] **W10-#1** e2e 통합 테스트 (회원가입 → step-up → 사기탐지 → 감사) 시나리오 통과 — ✅ 193dee4 (2026-05-10)
- [x] **W10-#2** 부하 테스트: P99 latency, Kafka 처리량, 감사 체인 1M 삽입 — ✅ 193dee4 (2026-05-10)
...
| 2026-05-10 | W10-#1 | e2e 통합 시나리오 (voice phishing → BLOCK → audit → SLO → chargeback → variant 별 precision/recall, fraud-service 단일 프로세스 내 시퀀스 자동화) + 4 PASS | 193dee4 |
| 2026-05-10 | W10-#2 | 부하 테스트 스크립트 (`scripts/loadtest/run.py` asyncio + aiohttp, p50/p95/p99 latency + success_rate + RPS, --n/--concurrency/--url CLI) + 4 PASS 정합성 | 193dee4 |
