...
| 주차 | 영역 | 총 | 완료 | % |
|---|---|---|---|---|
| W1 | 인증·인가 보안 (P0) | 7 | 1 | 14% |
| W2 | 챌린지·세션 외부화 (P0) | 7 | 0 | 0% |
...
- [x] **W1-#1** 알림 REST API `user_id` 쿼리스트링 → JWT 추출 (IDOR 차단) — `21_알림_시스템.md:204` — [하/보안] — ✅ (2026-05-02)
- [ ] **W1-#2** JWT 블랙리스트 메커니즘 (jti 기반, 로그아웃·계정잠금 시 무효화) — `perspectives/threat/01:77` — [상/보안]
- [ ] **W1-#3** JWT HS256 → RS256 전환 + kid 헤더 다중 키 라우팅 — `perspectives/threat/01:76` + `01_기본인증:214` — [상/보안]
- [ ] **W1-#4** ABE 마스터 시크릿 미설정 시 fail-fast 기동 중단 — `15_ABAC_ABE:517` — [하/보안]
- [ ] **W1-#5** ABAC 엔진 ↔ ABE 미들웨어 wiring (8가지 룰 런타임 연결) — `15_ABAC_ABE:512` — [상/보안]
- [ ] **W1-#6** 클라이언트 시그널 HMAC 서명 + 서버사이드 보강 — `14_행동_시그널:191` — [상/보안]
- [ ] **W1-#7** 403 응답에서 `required_policy`/`your_attributes` 마스킹 (운영 모드) — `15_ABAC_ABE:513` — [하/보안]
...
- [ ] **W2-#1** FIDO2 챌린지 3종 → Redis 단일 인터페이스 통합 — `02_FIDO2:360,364` — [중/인프라]
- [ ] **W2-#2** step-up 토큰 store → Redis hash — `12_실시간_사기:327` — [중/인프라]
- [ ] **W2-#3** 행동 시그널 DeviceStore → Redis sorted set + TTL — `14_행동_시그널:189` — [중/인프라]
- [ ] **W2-#4** 룰 토글 상태 → Redis/DB 영속화 — `13_규칙_엔진:254` — [중/인프라]
- [ ] **W2-#5** 위협 인텔 in-memory → PostgreSQL/Redis — `17_위협_인텔리전스:168` — [중/인프라]
...
| 2026-05-02 | W1-#1 | 알림 REST API IDOR 차단 (JWT 추출) | tbd |
