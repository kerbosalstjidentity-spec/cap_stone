...
- [x] **W7-#1** Feature drift 자동 탐지 (KS 검정 / 분위수 차이) — `mlops/04:87` + `threat/04:90` — [상/ML] — ✅ tbd (2026-05-10)
...
| 2026-05-10 | W7-#1 | Feature drift KS 검정 (`drift_detector.py` 양측 KS + Smirnov p-value 근사 + 분위수 차이, evaluate flow 가 amount/score live 기록 + `GET /admin/api/drift?threshold=0.2`) + 6 PASS / 회귀 17 PASS | tbd |
